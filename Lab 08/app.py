    ...
    with app.app_context():
        ...
        if not Student.query.first():
            ...
            enrollment1 = Enrollment(
                student_id=student1.id, course_id=course1.id, grade=85.5
            )
            enrollment2 = Enrollment(
                student_id=student1.id, course_id=course2.id, grade=92.0
            )
            enrollment3 = Enrollment(
                student_id=student2.id, course_id=course3.id, grade=88.0
            )
            ...
            print(
                f"📊 Students: {Student.query.count()}, "
            ...
            print(
                f"📊 Current counts - Students: {Student.query.count()}, "
                f"Teachers: {Teacher.query.count()}, "
                f"Courses: {Course.query.count()}, "
                f"Enrollments: {Enrollment.query.count()}"
            )
    ...
    if request.method == "POST":
        ...
        if student and student.check_password(password):
            ...
            return render_template(
                "student_login.html", error="Invalid email or password"
            )
...
def teacher_login():
    ...
    if request.method == "POST":
        ...
        if teacher and teacher.check_password(password):
            ...
            return render_template(
                "teacher_login.html", error="Invalid email or password"
            )
...
def student_dashboard():
    ...
    return render_template(
        "student_dashboard.html", student_name=student_name, courses=enrolled_courses
    )
...
def teacher_dashboard():
    ...
    return render_template(
        "professor_dashboard.html", teacher_name=teacher_name, courses=course_data
    )
...
def student_register():
    ...
    for course in course_data:
        print(
            f"   - {course['name']} by {course['professor']} "
...
def api_student_register():
    ...
    print(
        ...
        f"attempting to enroll in course {course_id}"
    )
...
# ----- Grades API for Lab 7-style grades page -----
        ...
        if student:
            ...
            if not enrollment:
                ...
                enrollment = Enrollment(
                    student_id=student.id, course_id=first_course.id
                )
...
def debug_courses():
    ...
    for course in courses:
        ...
        result += (
            f"<li>{course.name} - {teacher_name} - "
            f"{enrollment_count}/{course.capacity} (ID: {course.id})</li>"
        )
    ...
    for student in students:
        ...
        result += (
            f"<li>{student.name} - {student.email} "
            f"(Enrollments: {enrollments})</li>"
        )
